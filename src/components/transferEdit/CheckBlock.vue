        ...
        <div>
            ...
            <span v-if="needEdit">
                <el-checkbox v-model="currentData.checked" @change="handleChange(currentData)">
                    <span class="item-name" :title="currentData.alias" v-if="!isEdit">{{currentData.alias}}</span>
            ...
            <span v-if="!needEdit">
                <el-checkbox v-model="currentData.checked" @change="handleChange(currentData)">
                    <span class="item-name no-edit" :title="currentData[showFlied]">{{currentData[showFlied]}}</span>
                </el-checkbox>
            </span>
        </div>
    </div>
...
export default {
    //当前的数据，显示的字段名字，子字段的名字,direction表示方向
    props: ['currentData', 'showFlied', 'childFlid', 'needEdit','direction'],
    ...
    methods: {
        ...
        handleChange(data) {
            ...
            this.$emit('Itemchange',this.direction) //通知上级组件，当前多选元素改变了。
        },
        //下拉展开
        toggle() {
        ...
        //编辑
        edit(data) {
            this.inputVal = data.alias;
            this.isEdit = true;

        },
        //编辑保存
        save(data) {
            data.alias = this.inputVal;
            this.isEdit = false;
            this.$emit('watchSave');
        }
    },
