<template>
  <div class="user-box">
    ...
    <div class="ipt-btn-box" style="justify-content: end;">
      <el-input placeholder="主体名称" class="w250 seekIpt"
      ...
      <el-cascader class="selectHeight w250" :clearable="true" v-model="search.entityDeptIdList"
                   placeholder="辖区机构(全部)" :change-on-select="true"
    ...
    <div class="ipt-btn-box" style="justify-content: end;">
      ...
      <el-cascader class="selectHeight w250" :clearable="true" v-model="search.entryDeptList" placeholder="录入机构（全部）"
                   :options="deptList.options" :props="deptList.props" :change-on-select="true"
                   :show-all-levels="false" @change="opSearch">
      </el-cascader>

      <el-select clearable class="selectHeight" v-model="search.caseProperty" placeholder="案件性质（全部）"
                 @change="opSearch">
      ...
      <el-date-picker
        ...
        v-model="search.buildCaseTimeList"
        type="daterange"
        range-separator="至"
    ...
    <div class="tabOffTop170">
      <el-table :data="tableData" class="table-div"  @sort-change="handleSortChange">
        ...
        <el-table-column show-overflow-tooltip prop="entryDeptName"
                         label="录入机构"></el-table-column>
        <el-table-column show-overflow-tooltip prop="sourceStr"
                         label="线索来源"></el-table-column>
        <el-table-column show-overflow-tooltip prop="casePropertyStr"
                         label="案件性质"></el-table-column>
        <el-table-column show-overflow-tooltip prop="judgeContent"
                         label="判决内容"></el-table-column>
        <el-table-column show-overflow-tooltip prop="buildCaseTime" sortable="custom"
                         label="立案时间"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <a href="javascript:;"
               @click="showDetails(scope.row.id)" class="operate-a">详情</a>
          </template>
        </el-table-column>
      </el-table>
